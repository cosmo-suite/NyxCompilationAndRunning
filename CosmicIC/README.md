...
`input.par` - the inputs file. `np` is the number of cells in each direction.  
Note that the dimension in each direction specified by `box_size` is in the units of 
Mpc/h.  
